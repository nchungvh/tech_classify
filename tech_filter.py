...
def extract_abstract(entity):
    entity = entity.lower().replace(" ", "_").replace("#","").capitalize()
...
if __name__ == '__main__':
    ...
    regex_filtered_graph_dict = run_regex_filters(filter_regexes, graph_dict)
    print("#### Regex filtered")
    filtered_graph_dict = run_tech_classifier(classifier, regex_filtered_graph_dict, ent2abs, sent_model)

    with open(args.out + '/tech_filtered_' + args.gd + '.json', 'w', encoding = 'utf-8') as f:
        json.dump(filtered_graph_dict,f, ensure_ascii=False)    